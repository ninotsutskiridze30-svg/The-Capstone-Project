...
| 10% to 50% | Amber alert. Reliability items added to next sprint. No risky deployments without a rollback plan. |
...
**Who owns the budget freeze decision:** Luka Khimshiashvili
...
At current scale (MVP, small number of real users), these SLOs are targets, not yet based on measured production data. Full measurement requires Sprint 2 call-side PostHog event wiring. Until then, error budgets are verified via weekly manual smoke tests documented in `docs/standup-log.md`. The Vercel and LiveKit dashboards confirm zero infrastructure-level failures since deployment.
